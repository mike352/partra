...
/*****************************************************/
/**************Symmetric transfer matrices************/
/*****************************************************/

/*******************************/
unsigned char i_sq_f_r_s(unsigned char***** matrix, unsigned long long* msize, char* filename, const unsigned char N)
{
msize[1]=3ULL;
unsigned char umax=6*N-4;
sprintf(filename,"i_sq_f_r_s_%d",N);
...
lldiv_t bitfrac, bitfrac2;
unsigned char flip, flip2;
unsigned long long uh,uh2,rtotal=0ULL,ctotal=0ULL;
unsigned char count;
		...
		for (m=0;m<(1ULL<<N);m++)
		{
			...
			if (((bitarray[bitfrac2.quot]&(1<<bitfrac2.rem))>>bitfrac2.rem)==0) //bitarray unique configuration
			{
				...
				for (p=0;p<flip+1;p++)
				{
					...
					for (q=0;q<flip2+1;q++)
					{
						melement[(2*bit_sum(nn^mm)+uh+uh2)]++;
						mm=bit_reflection(mm,N);
					}
					nn=bit_reflection(nn,N);
				}
				...
				for (p=0;p<umax+1;p++)
				{
					if (melement[p]!=0)
					{
						//add normalization constant only to row vector
						(*matrix)[rtotal][ctotal][1][msize[1]*count]=p;
						(*matrix)[rtotal][ctotal][1][msize[1]*count+1]=melement[p];
						(*matrix)[rtotal][ctotal][1][msize[1]*count+2]=((1ULL+flip)*(1ULL+flip2));
						count++;
						melement[p]=0; //reset  - use memset once at end of loop? No, because this is more efficient, not all values are non-zero
					}
				}
...
/*******************************/
unsigned char i_sq_c_r_s(unsigned char***** matrix, unsigned long long* msize, char* filename, const unsigned char N)
{
msize[1]=3ULL;
unsigned char umax=6*N;
sprintf(filename,"i_sq_c_r_s_%d",N);
			...
			if (((bitarray[bitfrac3.quot]&(1<<bitfrac3.rem))>>bitfrac3.rem)==0) //bitarray unique configuration
			{
				...
				for (p=0;p<umax+1;p++)
				{
					if (melement[p]!=0)
					{
						...
						(*matrix)[rtotal][ctotal][1][msize[1]*count]=p;
						(*matrix)[rtotal][ctotal][1][msize[1]*count+1]=melement[p];
						(*matrix)[rtotal][ctotal][1][msize[1]*count+2]=((1ULL+flip)*(order[rtotal]+1ULL)*(1ULL+flip2)*(order[ctotal]+1ULL));
						count++;
						melement[p]=0; //reset  - use memset once at end of loop? No, because this is more efficient, not all values are non-zero
					}
				}
...
/*******************************/
unsigned char if_sq_f_r_s(unsigned char***** matrix, unsigned long long* msize, char* filename, const unsigned char N)
{
msize[1]=4ULL;
unsigned char umax=6*N-4;
unsigned char xmax=4*N;
sprintf(filename,"if_sq_f_r_s_%d",N);
...
lldiv_t bitfrac, bitfrac2;
unsigned char flip, flip2;
unsigned long long uh,uh2,xh,xh2,rtotal=0ULL,ctotal=0ULL;
unsigned char count;
		...
		for (m=0;m<(1ULL<<N);m++)
		{
			...
			if (((bitarray[bitfrac2.quot]&(1<<bitfrac2.rem))>>bitfrac2.rem)==0) //bitarray unique configuration
			{
				...
				for (p=0;p<flip+1;p++)
				{
					...
					for (q=0;q<flip2+1;q++)
					{
						melement[(2*bit_sum(nn^mm)+uh+uh2)][(xh+xh2)]++;
						mm=bit_reflection(mm,N);
					}
					nn=bit_reflection(nn,N);
				}
				...
				for (p=0;p<umax+1;p++)
				{
					for (q=0;q<xmax+1;q++)
					{
						if (melement[p][q]!=0)
						{
							...
							(*matrix)[rtotal][ctotal][1][msize[1]*count]=p;
							(*matrix)[rtotal][ctotal][1][msize[1]*count+1]=q;
							(*matrix)[rtotal][ctotal][1][msize[1]*count+2]=melement[p][q];
							(*matrix)[rtotal][ctotal][1][msize[1]*count+3]=((1ULL+flip)*(1ULL+flip2)); 
							count++;
							melement[p][q]=0; //reset  - use memset once at end of loop? No, because this is more efficient, not all values are non-zero
						}
					}
...
/*******************************/
unsigned char if_sq_c_r_s(unsigned char***** matrix, unsigned long long* msize, char* filename, const unsigned char N)
{
msize[1]=4ULL;
unsigned char umax=6*N;
unsigned char xmax=4*N;
sprintf(filename,"if_sq_c_r_s_%d",N);
				...
				for (p=0;p<umax+1;p++)
				{
					for (q=0;q<xmax+1;q++)
					{
						if (melement[p][q]!=0)
						{
							...
							(*matrix)[rtotal][ctotal][1][msize[1]*count+1]=q;
							(*matrix)[rtotal][ctotal][1][msize[1]*count+2]=melement[p][q];
							(*matrix)[rtotal][ctotal][1][msize[1]*count+3]=((1ULL+flip)*(order[rtotal]+1ULL)*(1ULL+flip2)*(order[ctotal]+1ULL)); 
							melement[p][q]=0; //reset  - use memset once at end of loop? No, because this is more efficient, not all values are non-zero
							count++;
						}
					}
