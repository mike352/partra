...
for(n=0; n<(1ULL<<N); n++)
{
	...
	for(m=0; m<(1ULL<<N); m++)
	{
		...
		(*matrix)[n][m][0][0]=1;
		(*matrix)[n][m][1][0]=2*bit_sum(n^m)+2*bit_sum((~1ULL&n)^(~1ULL&circ_single_lshift(m,N)))+uh+uh2;
		(*matrix)[n][m][1][1]=1;
	}
}
...
for(n=0; n<(1ULL<<N); n++)
{
	...
	for(m=0; m<(1ULL<<N); m++)
	{
		...
		(*matrix)[n][m][0][0]=1;
		(*matrix)[n][m][1][0]=2*bit_sum(n^m)+2*bit_sum(n^circ_single_lshift(m,N))+uh+uh2;
		(*matrix)[n][m][1][1]=1;
	}
}
